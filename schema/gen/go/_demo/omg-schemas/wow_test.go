...
func init() {
	...
	adjCfg := &gengo.AdjunctCfg{
		FieldSymbolLowerOverrides: map[gengo.FieldTuple]string{
			{"StructField", "type"}: "typ", // fixme: these don't appear to work, but certainly should
			{"TypeEnum", "type"}:    "typ",
		},
	...
	ts.Accumulate(schema.SpawnStruct("StructField",
		[]schema.StructField{
			schema.SpawnStructField("typ", "TypeDefnInline", false, false),
	...
	ts.Accumulate(schema.SpawnStruct("TypeEnum",
		[]schema.StructField{
			schema.SpawnStructField("typ", "Map__EnumValue__Unit", false, false), // todo: dodging inline defn's again.  also: this says unit; schema-schema does not.  schema-schema needs revisiting on this subject.
	...
	ts.Accumulate(schema.SpawnStruct("Unit", // todo: this is a heck of a stand-in.  we should introduce unit as first class type kind.
		[]schema.StructField{},
		schema.StructRepresentation_Map{},
	))
